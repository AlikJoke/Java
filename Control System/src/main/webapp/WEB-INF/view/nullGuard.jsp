...
<body bgcolor="#c0c0c0">
	...
	<sec:authorize access="hasRole('ROLE_GUARD')">
		<form:form action="ShowData" method="POST" commandName="tempVisitor" modelAttribute="tempVisitor" >
			<table>
		...
		<c:forEach items="${permanentVisitorList}" var="permanentVisitor">
			<tr>
				...
				<td align="center" bgcolor="white">${tempVisitor.phoneNumber}<br/>${tempVisitor.phoneNumber2}<br/>${permanentVisitor.phoneNumber3}</td>
	...
	<table border="2" width="1300px" cellpadding="8px" style="border-collapse: collapse">
		...
		<c:forEach items="${tempVisitorListIn}" var="tempVisitor">
			<tr>
				<td align="center" bgcolor="white">${tempVisitor.carNumber}</td>
				<td align="center" bgcolor="white">${tempVisitor.brand}</td>
				<td align="center" bgcolor="white"><form:input id="text" style="width:250px;height:25px;background-color: transparent;" path="realNumber"/></td>
				<td align="center" bgcolor="white"><button class="button10" id="search" type="submit" name="action" value="${tempVisitor.visitorId}.${tempVisitor.status}" style="width:125px">${tempVisitor.status}</button></td>
		...
		<c:forEach items="${tempVisitorListOut}" var="tempVisitor">
			<tr>
				<td align="center" bgcolor="white">${tempVisitor.carNumber}</td>
				<td align="center" bgcolor="white">${tempVisitor.brand}</td>
				<td align="center" bgcolor="white">${tempVisitor.realNumber}</td>
				<td align="center" bgcolor="white"><button id="search" class="button10" type="submit" name="action" value="${tempVisitor.visitorId}.${tempVisitor.status}" style="width:125px">${tempVisitor.status}</button></td>
				<td align="center" bgcolor="white">${tempVisitor.aim}</td>
				<td align="center" bgcolor="white">${tempVisitor.nameInf}, ${tempVisitor.address}<br/>${tempVisitor.phoneNumber}<br/>${tempVisitor.phoneNumber2}<br/>${tempVisitor.phoneNumber3}</td>		
			</tr>
		</c:forEach>
